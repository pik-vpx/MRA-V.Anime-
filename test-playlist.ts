import { findAnimeForTrack } from './src/lib/api.ts';
...
function parseEntries(text: string) {
  const entries: { title: string; artist: string; anime: string }[] = [];
  const regex = /([^\n]+?)\s+by\s+([^\n]*?)\s+OP[\d\w-]*\s*•\s*([^\n]+)/gi;
...
async function runTests() {
  console.log('=== Testing Top 100 Anime Themes from Playlist ===\n');
